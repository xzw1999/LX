			...
			<view class="list" v-show="active === 1">
				<view class="item acea-row row-between-wrapper">
					...
					<view class="select-box" v-if="postPeople.length>0">
						<picker class="pickerBox" @change="bindPostChange" :value="postIndex" :range="postPeople" range-key="nickname">
							<!-- <view></view> -->
							<view class="uni-input">{{postPeople[postIndex].nickname}}</view>
						</picker>
					</view>
				</view>
			</view>
			...
			scanCode() {
				...
				if (this.$wechat.isWeixin()) {
					this.$wechat.wechatEvevt('scanQRCode', {
						...
						this.delivery_id = res.resultStr;
					});
				}
				// #endif
			},

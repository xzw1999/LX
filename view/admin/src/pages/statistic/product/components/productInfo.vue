<template>
    <Card :bordered="false" dis-hover class="ivu-mt">
        <div class="acea-row row-between-wrapper mb20">
            <div class="header-title">商品概括
                <Poptip word-wrap width="500" trigger="hover"  placement="right-start">
                    <Icon type="ios-information-circle-outline" />
                    <div slot="content">
                        <div>商品浏览量</div>
                        <div>在选定条件下，所有商品详情页被访问的次数，一个人在统计时间内访问多次记为多次</div><br>
                        <div>商品访客数</div>
                        <div>在选定条件下，访问任何商品详情页的人数，一个人在统计时间范围内访问多次只记为一个</div><br>
                        <div>加购件数</div>
                        <div>在选定条件下，添加商品进入购物车的商品件数</div><br>
                        <div>下单件数</div>
                        <div>在选定条件下，成功下单的商品件数之和（拼团商品在成团之后计入，线下支付订单在后台确认支付后计入，不剔除退款订单）</div><br>
                        <div>支付件数</div>
                        <div>在选定条件下， 成功付款订单的商品件数之和（拼团商品在成团之后计入，线下支付订单在后台确认支付后计入，不剔除退款订单）</div><br>
                        <div>支付金额</div>
                        <div>在选定条件下， 成功付款订单的商品金额之和（拼团商品在成团之后计入，线下支付订单在后台确认支付后计入，不剔除退款订单）</div><br>
                        <div>成本金额</div>
                        <div>在选定条件下，成功付款订单的商品成本金额之和</div><br>
                        <div>退款金额</div>
                        <div>在选定条件下，成功退款的商品金额之和</div><br>
                        <div>退款件数</div>
                        <div>在选定条件下，成功退款的商品件数之和</div><br>
                        <div> 访客 - 支付转化率</div>
                        <div>在选定条件下， 付款人数 / 访客数</div><br>
                    </div>
                </Poptip>
            </div>
            <div class="acea-row">
                <DatePicker :editable="false" :clearable="false" @on-change="onchangeTime" :value="timeVal" format="yyyy/MM/dd"
                            type="datetimerange" placement="bottom-start" placeholder="自定义时间"
                            style="width: 200px;" class="mr15" :options="options"></DatePicker>
        ...
        <div class="acea-row mb20">
            <div class="infoBox acea-row mb30" v-for="(item, index) in list" :key="index">
                <div class="iconCrl mr15" :class="{'one':index%4==0,'two':index%4==1,'three':index%4==2,'four':index%4==3}"><i :class="item.icon" class="iconfont"></i></div>
                <div class="info">
                    ...
                    <span class="content-time spBlock">环比增长：<i class="content-is" :class="Number(item.list.percent)>=0?'up':'down'">{{ item.list.percent }}%</i><Icon :color="Number(item.list.percent)>=0?'#F5222D':'#39C15B'" :type="Number(item.list.percent)>=0?'md-arrow-dropup':'md-arrow-dropdown'" /></span>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 统计
            getStatistics() {
                statisticBasicApi({data: this.dataTime}).then(async res => {

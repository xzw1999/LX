<template>
    <div>
        <Card :bordered="false" dis-hover>
            ...
            <!--商品采集，物流，电子面单列表-->
            <div v-else-if="(isChecked === '3' && query.open === 1) || (isChecked === '4' && copy.open === 1) || (isChecked === '2' && dump.open === 1) ">
            ...
            <div v-else>
                ...
                <!--电子面单立即开通-->
                <div class="smsBox" v-if="isDump && (isChecked === '2' || isChecked === '3')">
                    <div class="index_from page-account-container">
                        <div class="page-account-top">
                            <span class="page-account-top-tit">开通短信服务</span>
                        </div>
                        <Form ref="formInlineDump" :model="formInlineDump" :rules="ruleInlineDump" @submit.native.prevent @keyup.enter="handleSubmitDump('formInlineDump')">
                            <FormItem prop="com" class="maxInpt">
                                <Select v-model="formInlineDump.com"  placeholder="请选择快递公司" @on-change="onChangeExport" style="text-align: left;">
                            ...
                            <FormItem prop="temp_id" class="tempId maxInpt">
                                <div class="acea-row">
                                    <Select v-model="formInlineDump.temp_id"  placeholder="请选择电子面单模板" style="text-align: left;" :class="[formInlineDump.temp_id?'width9':'width10']" @on-change="onChangeImg">
                                        <Option v-for="(item, index) in exportTempList" :value="item.temp_id" :key="index">{{ item.title }}</Option>
                                    </Select>
                            ...
                            <FormItem class="maxInpt">
                                <Button type="primary" long size="default" @click="handleSubmitDump('formInlineDump')" class="btn">{{$t('page.login.submit') }}</Button>
                            </FormItem>
                        </Form>
                    </div>
                </div>

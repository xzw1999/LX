...
class ProductController
{
    ...
    public function getRecommendList(Request $request, $type)
    {
        ...
        $where['is_del'] = 0;
        if ($type == 1) {//TODO 精品推荐
            $data['list'] = $product->getRecommendProduct($request->uid(), 'is_best');//TODO 精品推荐个数
            $where['is_best'] = 1;
        } else if ($type == 2) {//TODO  热门榜单
            $data['list'] = $product->getRecommendProduct($request->uid(), 'is_hot');//TODO 热门榜单 猜你喜欢
            $where['is_hot'] = 1;
        } else if ($type == 3) {//TODO 首发新品
            $data['list'] = $product->getRecommendProduct($request->uid(), 'is_new');//TODO 首发新品
            $where['is_new'] = 1;
        } else if ($type == 4) {//TODO 促销单品
            $data['list'] = $product->getRecommendProduct($request->uid(), 'is_benefit');//TODO 促销单品
            $where['is_benefit'] = 1;
        }

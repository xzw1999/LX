...
class PublicController
{
    ...
    public function menu_user(Request $request)
    {
        ...
        $userService = $invoiceStatus = $deliveryUser = $isUserPromoter = $userVerifyStatus = true;

        if ($uid && $userInfo) {
            /** @var StoreServiceServices $storeService */
            $storeService = app()->make(StoreServiceServices::class);
            $userService = $storeService->checkoutIsService($uid);
        ...
        foreach ($menusInfo as $key => &$value) {
            ...
            if ($value['id'] == 135 && !$vipOpen)
                unset($menusInfo[$key]);
            if ($value['id'] == 137 && (!$brokerageFuncStatus || !($isUserPromoter)))
                unset($menusInfo[$key]);
            if ($value['id'] == 138 && !$balanceFuncStatus)
                unset($menusInfo[$key]);
            if ($value['id'] == 174 && !$userService)
                unset($menusInfo[$key]);
            if ((((!$userService) && (!$userVerifyStatus)) && !$deliveryUser) && $value['url'] === '/pages/admin/order_cancellation/index')
                unset($menusInfo[$key]);
            /*if (!$deliveryUser && $value['url'] === '/pages/admin/order_cancellation/index')
                unset($menusInfo[$key]);*/
            if ((!$userService) && $value['url'] === '/pages/admin/order/index')
                unset($menusInfo[$key]);
            if ($value['url'] == '/pages/users/user_vip/index' && !$vipOpen)
                unset($menusInfo[$key]);
            if ($value['url'] == '/pages/users/user_invoice_list/index' && !$invoiceStatus)
                unset($menusInfo[$key]);
            if ($value['id'] == 251 && !$vipCard)
                unset($menusInfo[$key]);
            if ($value['id'] == 253 && !$userService) {
                unset($menusInfo[$key]);
            } elseif ($value['id'] == 253 && $userService) {
                $value['url'] = sys_config('site_url') . $value['url'];
            }
            if ($uid == 0) {
                if ($value['id'] == 137 || $value['id'] == 174 || $value['id'] == 176 || $value['id'] == 253) unset($menusInfo[$key]);
            }
        }
    ...
    public function pink(StorePinkServices $pink, UserServices $user)
    {
        ...
        $data['avatars'] = $uids ? $user->getColumn([['uid', 'in', $uids]], 'avatar') : [];
        return app('json')->successful($data);
    }

...
class StoreServiceServices extends BaseServices
{
    ...
    {
        if ($formData) {
            $field[] = $this->builder->frameImage('avatar', '客服头像', $this->url('admin/widget.images/index', ['fodder' => 'avatar'], true), $formData['avatar'] ?? '')->icon('ios-add')->width('60%')->height('435px');
        } else {
    ...
    {
        return $this->dao->count(['uid' => $uid, 'status' => 1, 'is_tourist' => 0, 'customer' => 1]) ? true : false;
    }

...
//            sort($item['detail'], SORT_STRING);
    ...
    public function getBargain(Request $request, int $id)
    {
        ...
        $data['bargainSumCount'] = $orderService->count(['bargain_id' => $id, 'paid' => 1, 'uid' => $user['uid'], 'refund_status' => 0]);
        ...
        $storeVisit->setView($user['uid'], $id, 'bargain', $bargain['product_id'], 'view');
        //浏览记录
        Queue::instance()->job(ProductLogJob::class)->data('visit', ['uid' => $user['uid'], 'product_id' => $bargain['product_id']])->push();
        return $data;
    }
        ...
        if ($res) {
            if (!$bargainUserService->getSurplusPrice($bargainUserTableId, 1)) {
                ...
                if ($userOpenid['user_type'] == 'wechat') {
                    Queue::instance()->do('sendBargainSuccess')->job(WechatTemplateJob::class)->data($userOpenid['openid'], $bargainInfo, $bargainUserInfo, $bargainUserUid)->push();

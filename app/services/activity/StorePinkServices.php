...
class StorePinkServices extends BaseServices
{
    ...
    public function orderPinkAfterNo($uid, $pid, $isRemove = false, $channel)
    {
        ...
        if ($channel == 1) {
            ...
            if ($isRemove) {
                Queue::instance()->do('sendPinkFail')->job(RoutineTemplateJob::class)->data($userOpenid, $pink->title, $pink->people, '亲，您的拼团取消，点击查看订单详情', '/pages/order_details/index?order_id=' . $pink->order_id)->push();
            } else {
                Queue::instance()->do('sendPinkFail')->job(RoutineTemplateJob::class)->data($userOpenid, $pink->title, $pink->people, '亲，您拼团失败，自动为您申请退款，退款金额为：' . $pink->price, '/pages/order_details/index?order_id=' . $pink->order_id)->push();
            }
        } elseif ($channel == 0) {
            $userOpenid = $wechatService->value(['uid' => $uid, 'user_type' => 'wecaht'], 'openid');
            if ($isRemove) {
                Queue::instance()->do('sendOrderPinkClone')->job(WechatTemplateJob::class)->data($userOpenid, $pink, $pink->title)->push();
            } else {
                Queue::instance()->do('sendOrderPinkFial')->job(WechatTemplateJob::class)->data($userOpenid, $pink, $pink->title)->push();
            }
        }
    ...
    public function orderPinkAfter($uidAll, $pid)
    {
        ...
        foreach ($pinkList as $item) {
            ...
            if ($order_channels[$item['order_id']] == 1) {
                ...
                Queue::instance()->do('sendPinkSuccess')->job(RoutineTemplateJob::class)->data($userOpenid, $title, $item['nickname'], $item['add_time'], $item['people'], '/pages/users/order_details/index?order_id=' . $item['order_id'])->push();
            } elseif ($order_channels[$item['order_id']] == 0) {
                $userOpenid = $wechatService->value(['uid' => $item['uid'], 'user_type' => 'wechat'], 'openid');
                Queue::instance()->do('sendOrderPinkSuccess')->job(WechatTemplateJob::class)->data($userOpenid, $item['order_id'], $item['id'], $title)->push();
            }
        }
    ...
    public function joinPinkSuccessSend(array $order, string $openid, string $title, $pink)
    {
        if ($order['is_channel'] == 1) {
            ...
            return Queue::instance()->do('sendPinkSuccess')->job(RoutineTemplateJob::class)
                ->data($openid, $title, $nickname, $pink['add_time'], $pink['people'], '/pages/users/order_details/index?order_id=' . $pink['order_id'])->push();
        } else {

...
//            sort($item['detail'], SORT_STRING);
    ...
    public function seckillDetail(Request $request, int $id)
    {
        ...
        $storeInfo['code_base'] = $qrcodeService->getWechatQrcodePath($id . '_product_seckill_detail_wap.jpg', '/pages/activity/goods_seckill_details/index?id=' . $id);
...
//        $storeInfo['like_num'] = $storeProductRelationServices->productRelationNum((int)$id, 'like', 'product_seckill');
        ...
        $storeVisit->setView($uid, $id, 'seckill', $storeInfo['product_id'], 'view');
        //浏览记录
        Queue::instance()->job(ProductLogJob::class)->data('visit', ['uid' => $uid, 'product_id' => $storeInfo['product_id']])->push();
        return $data;
    }

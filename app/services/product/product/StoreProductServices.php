...
//            }
    ...
    public function checkActivity(int $id = 0)
    {
        if ($id != 0) {
            ...
            if ($res1 || $res2 || $res3) throw new AdminException('该商品有活动开启，无法编辑属性');
        } else {
     ...
     * @return bool
     */
    public function vipIsOpen(bool $vip = false)
    {
        /** @var MemberCardServices $memberCardService */
        $memberCardService = app()->make(MemberCardServices::class);
        $vipStatus = $memberCardService->isOpenMemberCard('vip_price');
        return $vipStatus && sys_config('member_func_status', false) && $vip;
    }
     ...
     */
    public function setLevelPrice($list, int $uid, bool $isSingle = false, $discount = 0, $vipPrice = 0.00, $is_vip = 0, $is_show = false)
    {
        /** @var MemberCardServices $memberCardService */
        $memberCardService = app()->make(MemberCardServices::class);
        $vipStatus = $memberCardService->isOpenMemberCard('vip_price');
        ...
        if ($uid) {
            ...
            $discount = $discount !== 0 ? $discount : $userLevelServices->getDiscount($uid, 'discount');
            $isPayLevel = $userService->get($uid);
            $isMember = $isPayLevel['is_money_level'] > 0 ? true : false;
        } else {
            //没登录
            $discount = 100;
            $isMember = false;
        ...
        if (!is_array($list)) {
            ...
            return $this->isPayLevelPrice($uid, $list, $discount, $isSingle, $vipPrice, $is_vip, $is_show);
        }
        //当$list为数组时$isSingle==true为一维数组 ，否则为二维
        if ($isSingle) {
            $vipPrice = $this->isPayLevelPrice($uid, $list, $discount, $isSingle, $vipPrice, $is_vip, $is_show);
            if ($vipStatus && $is_vip/* && $isMember*/) {
                $list['vip_price'] = $vipPrice;
            } else {
                $list['vip_price'] = 0.00;
            }
        } else {
            //诸如精品推荐，会员推荐，打折促销价格核算走这里
            foreach ($list as &$item) {
                $is_vip = $vipStatus ? $item['is_vip'] : 0;
                $vipPrice = $this->isPayLevelPrice($uid, $item, $discount, $isSingle, $item['vip_price'], $is_vip, $is_show);
                if ($vipStatus && $is_vip/* && $isMember*/) {
                    $item['vip_price'] = $vipPrice;
                } else {
                    $item['vip_price'] = 0.00;
                }
            }
    ...
    public function getRecommendProduct(int $uid, $field, int $num = 0)
    {
        ...
        $memberCardService = app()->make(MemberCardServices::class);
        $vipStatus = $memberCardService->isOpenMemberCard('vip_price');
        $isVipOpen = $vipStatus && sys_config('member_func_status', false);
        foreach ($list as &$item) {
            if (!($isVipOpen && $item['is_vip'])) {
                $item['vip_price'] = 0;
            }
        }
        return $list;
     ...
     * @return float|string
     */
    public function isPayLevelPrice(int $uid, $goodsList, string $discount, $isSingle = false, $payVipPrice = 0.00, $is_vip = 0, $is_show = false)
    {
        ...
        $userService = app()->make(UserServices::class);
        $isPayLevel = $userService->get($uid);
        $price = !is_array($goodsList) ? $goodsList : $goodsList['price'];
        $noPayVipPrice = ($discount && $discount != 0.00 && sys_config('member_func_status')) ? bcmul((string)$discount, (string)$price, 2) : $price;
        $vipPrice = ($payVipPrice < $noPayVipPrice && $payVipPrice > 0) ? $payVipPrice : $noPayVipPrice;
        //看是否开启付费会员优惠
        /** @var MemberCardServices $memberCardService */
        $memberCardService = app()->make(MemberCardServices::class);
        $vipStatus = $memberCardService->isOpenMemberCard('vip_price');
        if (!is_array($goodsList)) {
            ...
            if ($vipStatus && $is_vip == 1) {
                ...
                if (!$is_show) {
                    if ($isSingle) return $vipPrice;
                    return bcsub((string)$goodsList, (string)$vipPrice, 2);
                } else {
                    if ($isPayLevel['is_money_level'] > 0) {
                        if ($isSingle) return $vipPrice;
                        return bcsub((string)$goodsList, (string)$vipPrice, 2);
                    } else {
                        if ($isSingle) return $noPayVipPrice;
                        return bcsub((string)$goodsList, (string)$noPayVipPrice, 2);
                    }
                }
            ...
            if ($vipStatus && $is_vip == 1) {
                if (!$is_show) {
                    ...
                    if ($isPayLevel['is_money_level'] > 0) return $vipPrice;
                    return $noPayVipPrice;
                }
            }
            return $noPayVipPrice;
        }

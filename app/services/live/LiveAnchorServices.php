...
use app\dao\live\LiveAnchorDao;
...
class LiveAnchorServices extends BaseServices
{
    ...
    public function add(int $id)
    {
        ...
        if (!$anchor) {
            ...
            $field[] = Form::frameImage('cover_img', '主播图像', Url::buildUrl('admin/widget.images/index', array('fodder' => 'cover_img')), '')->icon('ios-add')->width('60%')->height('435px')->appendValidate(Validate::str()->required('请选择图像'));
        } else {
            $title = '修改主播';
            ...
            $field[] = Form::frameImage('cover_img', '主播图像', Url::buildUrl('admin/widget.images/index', array('fodder' => 'cover_img')), $anchor->getData('cover_img'))->icon('ios-add')->width('60%')->height('435px')->appendValidate(Validate::str()->required('请选择图像'));
        }
    ...
    public function syncAnchor($is_job = false)
    {
        ...
        if (!$res || $is_job) {
            ...
            //支付成功后发送消息
            if (!$is_job) Queue::instance()->job(\crmeb\jobs\LiveJob::class)->secs(120)->push();

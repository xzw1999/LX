...
class ProductStatisticServices extends BaseServices
{
    ...
    public function trend($time, $num, $excel = false)
    {
        ...
        $refund = array_column($storeOrder->getProductTrend($time, $timeType, 'refund_reason_time', 'sum(refund_price)'), 'num', 'days');
        if ($excel) {
            ...
            $refundNum = array_column($storeOrder->getProductTrend($time, $timeType, 'refund_reason_time', 'sum(total_num)'), 'num', 'days');
            foreach ($xAxis as &$item) {
                if (isset($user[$item]) && isset($payNum[$item])) {
                    $changes = bcmul(bcdiv((string)$payNum[$item], (string)$user[$item], 4), 100, 2);
